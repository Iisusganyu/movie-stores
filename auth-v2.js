...
function updateLoginButton(username) {
    ...
    console.log('Обновляем кнопку входа. Пользователь:', username, 'Найдено кнопок:', loginButtons.length);

    loginButtons.forEach(btn => {
        console.log('Обрабатываем кнопку:', btn);
        ...
        if (username) {
            ...
            // Для ссылок
            if (btn.tagName === 'A') {
                btn.href = '#'; // Убираем переход на auth.html
                console.log('Убрал href у ссылки');
            }

            // Добавляем класс для стилизации
            btn.classList.add('user-logged-in');
            console.log('Добавил класс user-logged-in');

            // Убираем все старые обработчики
            btn.onclick = null;

            // Добавляем новый обработчик для показа меню
